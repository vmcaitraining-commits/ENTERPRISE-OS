...
export const CeoBriefVisual: React.FC = () => {
  ...
  const localizedAnomalies = tRaw<any[]>('aiCopilots.visuals.ceo.anomalies');
  const anomalies = Array.isArray(localizedAnomalies) && localizedAnomalies.length > 0 
    ? localizedAnomalies 
    : fallbackAnomalies;

  const currentAnomaly = anomalies[activeAnomaly] || anomalies[0];
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300">
              <Clock className="w-3 h-3 mr-1" /> {t('aiCopilots.visuals.ceo.simulatedHeader', '[Kịch bản minh họa] Hỗ trợ tổng hợp 07:30 AM khi kết nối dữ liệu')}
            </span>
            <span className="text-xs text-slate-500 dark:text-slate-400">
              {t('aiCopilots.visuals.ceo.reconciledSubheader', 'Đối soát từ 4 phân hệ khi được cấu hình')}
            </span>
            <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300">
              {t('aiCopilots.visuals.ceo.simulatedBadge', '[Dữ liệu minh họa]')}
            </span>
          </div>
          <h4 className="text-lg font-bold text-slate-900 dark:text-white mt-1">
            {t('aiCopilots.visuals.ceo.title', 'Bản Tin Điều Hành Sáng (Morning Executive Brief)')}
          </h4>
        </div>
        <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
          <ShieldAlert className="w-4 h-4 text-primary" /> {t('aiCopilots.visuals.ceo.rbacNotice', 'Phân quyền bảo mật: Chỉ dành cho Ban Tổng Giám Đốc')}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 my-5">
        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/70 dark:border-slate-800">
          <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
            <span>{t('aiCopilots.visuals.ceo.kpis.revenue.label', 'Doanh thu MTD [Minh họa]')}</span>
            <TrendingUp className="w-3.5 h-3.5 text-emerald-600" />
          </div>
          <div className="text-base sm:text-lg font-bold text-slate-900 dark:text-white mt-1">
            {t('aiCopilots.visuals.ceo.kpis.revenue.value', '3.82 / 4.15 Tỷ')}
          </div>
          <div className="text-xs text-emerald-600 font-medium mt-0.5">
            {t('aiCopilots.visuals.ceo.kpis.revenue.note', 'Đạt 92.0% kế hoạch tháng')}
          </div>
        </div>

        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/70 dark:border-slate-800">
          <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
            <span>{t('aiCopilots.visuals.ceo.kpis.cashflow.label', 'Dòng tiền D+21 [Minh họa]')}</span>
            <DollarSign className="w-3.5 h-3.5 text-amber-600" />
          </div>
          <div className="text-base sm:text-lg font-bold text-amber-600 dark:text-amber-400 mt-1">
            {t('aiCopilots.visuals.ceo.kpis.cashflow.value', '- 420 Triệu VNĐ')}
          </div>
          <div className="text-xs text-amber-600 font-medium mt-0.5">
            {t('aiCopilots.visuals.ceo.kpis.cashflow.note', 'Cảnh báo thiếu hụt ngắn hạn')}
          </div>
        </div>

        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/70 dark:border-slate-800">
          <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
            <span>{t('aiCopilots.visuals.ceo.kpis.sla.label', 'SLA Công việc [Minh họa]')}</span>
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
          </div>
          <div className="text-base sm:text-lg font-bold text-slate-900 dark:text-white mt-1">
            {t('aiCopilots.visuals.ceo.kpis.sla.value', '94.2% Đúng hạn')}
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {t('aiCopilots.visuals.ceo.kpis.sla.note', '380/403 tác vụ tuần')}
          </div>
        </div>
        ...
        <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-900/60">
          <div className="text-xs text-amber-800 dark:text-amber-300 flex items-center justify-between font-semibold">
            <span>{t('aiCopilots.visuals.ceo.kpis.anomalies.label', 'Bất thường cần quyết định [Minh họa]')}</span>
            <AlertTriangle className="w-3.5 h-3.5 text-amber-600" />
          </div>
          <div className="text-base sm:text-lg font-bold text-amber-900 dark:text-amber-200 mt-1">
            {t('aiCopilots.visuals.ceo.kpis.anomalies.value', '02 Điểm chốt')}
          </div>
          <div className="text-xs text-amber-700 dark:text-amber-400 font-medium mt-0.5">
            {t('aiCopilots.visuals.ceo.kpis.anomalies.note', 'Cần Lãnh đạo chỉ đạo')}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h5 className="text-sm font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-amber-500" /> {t('aiCopilots.visuals.ceo.anomaliesListTitle', 'Danh sách điểm bất thường [Kịch bản & dữ liệu minh họa]:')}
          </h5>
          <div className="text-xs text-slate-500">
            {t('aiCopilots.visuals.ceo.clickInstruction', 'Bấm để xem chi tiết & duyệt phương án')}
          </div>
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-xs text-rose-600 dark:text-rose-400 font-medium">
                  {t('aiCopilots.visuals.ceo.riskWarningBadge', 'Cảnh báo rủi ro [Minh họa]')}
                </span>
              </div>
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                {t('aiCopilots.visuals.ceo.impactLabel', 'Tác động:')} <strong className="text-slate-700 dark:text-slate-300">{item.impact}</strong>
              </div>
            </button>
          ))}
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <div className="text-xs font-medium text-primary uppercase tracking-wider">
                {t('aiCopilots.visuals.ceo.proposedActionHeader', 'Đề xuất hành động từ kịch bản AI minh họa (để Lãnh đạo phê duyệt):')}
              </div>
              <p className="text-sm text-slate-800 dark:text-slate-200 font-medium mt-1">
                {currentAnomaly?.suggestedAction}
              </p>
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1.5">
                <UserCheck className="w-3.5 h-3.5 text-slate-400" /> {t('aiCopilots.visuals.ceo.approvalCheckpointNotice', 'Điểm chốt chặn: AI không tự phát thông báo. Chỉ gửi khi Tổng Giám đốc xác nhận.')}
              </div>
            </div>
            ...
            {decisionState === 'pending' ? (
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors cursor-pointer"
                >
                  {t('aiCopilots.visuals.ceo.dismissBtn', 'Bỏ qua')}
                </button>
                <button
                  onClick={() => setDecisionState('approved')}
                  className="px-4 py-2 text-xs font-medium text-white bg-primary hover:bg-primary-hover rounded-lg flex items-center gap-1.5 transition-colors shadow-sm cursor-pointer"
                >
                  <Bell className="w-3.5 h-3.5" /> {t('aiCopilots.visuals.ceo.approveBtn', 'Phê duyệt gửi thông báo')}
                </button>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-3 py-2 rounded-lg border border-emerald-200 dark:border-emerald-800">
                <CheckCircle2 className="w-4 h-4" /> {t('aiCopilots.visuals.ceo.approvedNotice', 'Đã gửi thông báo triệu tập họp đến 2 phòng ban')}
              </div>
            ) : (
              <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-200/60 dark:bg-slate-700/50 px-3 py-2 rounded-lg">
                {t('aiCopilots.visuals.ceo.dismissedNotice', 'Đã ghi nhận bỏ qua cảnh báo này')}
              </div>
            )}
          </div>
        </div>
