...
import { 
  Headphones, BookOpen, CheckCircle2, Send, Edit3, 
...
export const CsTicketVisual: React.FC = () => {
  ...
  const defaultText = t(
    'aiCopilots.visuals.customerService.defaultResponseText',
  ...
  const [customResponseText, setCustomResponseText] = useState<string | null>(null);
  const responseText = customResponseText !== null ? customResponseText : defaultText;
    ...
    <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-6 shadow-sm space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800 gap-2">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-bold text-slate-900 dark:text-white text-sm">
                {t('aiCopilots.visuals.customerService.ticketTitle', 'Ticket #CS-8492: Khiếu nại tiến độ giao linh kiện')}
              </span>
              <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300">
                {t('aiCopilots.visuals.customerService.priorityBadge', 'Ưu tiên P2 (Khẩn cấp)')}
              </span>
              <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                {t('aiCopilots.visuals.customerService.simulatedBadge', '[Dữ liệu minh họa]')}
              </span>
            </div>
            <div className="text-xs text-slate-500 flex flex-wrap items-center gap-3 mt-0.5">
              <span>{t('aiCopilots.visuals.customerService.customerLabel', 'Khách hàng: Nguyễn Hoàng Nam')}</span>
              <span>•</span>
              <span>{t('aiCopilots.visuals.customerService.orderLabel', 'Đơn hàng: #LK-992 (24.500.000 VNĐ)')}</span>
              <span>•</span>
              <span className="text-rose-500 font-medium">
                {t('aiCopilots.visuals.customerService.sentimentLabel', 'Sắc thái theo kịch bản: Bức xúc [Gợi ý tham khảo]')}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="text-xs text-slate-500 flex items-center gap-1">
          ...
          {t('aiCopilots.visuals.customerService.intakeTime', '[Minh họa] Tiếp nhận: 14:20 (Chờ 4 phút)')}
        </div>
      </div>
      ...
      <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
        <span className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">
          {t('aiCopilots.visuals.customerService.complaintHeader', 'Nội dung khách hàng gửi qua Web Portal [Kịch bản minh họa]:')}
        </span>
        <p className="text-slate-600 dark:text-slate-400 italic">
          {t('aiCopilots.visuals.customerService.complaintQuote', '"Đơn linh kiện hẹn giao từ hôm qua mà nay vẫn chưa thấy đâu, gọi tổng đài không ai nhấc máy. Công trình của bên tôi đang bị đình trệ, nếu chiều nay không giao kịp tôi sẽ hủy toàn bộ hợp đồng và yêu cầu bồi thường!"')}
        </p>
      </div>
      ...
      <div className="p-3 bg-indigo-50/60 dark:bg-indigo-950/20 border border-indigo-200/70 dark:border-indigo-900/40 rounded-xl text-xs space-y-1">
        <div className="flex items-center justify-between font-semibold text-indigo-900 dark:text-indigo-300">
          <span className="flex items-center gap-1.5">
            ...
            {t('aiCopilots.visuals.customerService.citationHeader', 'Căn cứ quy chế trích dẫn [Mô phỏng nguồn tài liệu]:')}
          </span>
          <span className="text-[11px] text-indigo-600 dark:text-indigo-400 underline cursor-pointer flex items-center gap-0.5">
            {t('aiCopilots.visuals.customerService.viewOriginal', 'Xem tài liệu gốc')} <ExternalLink className="w-3 h-3" />
          </span>
        </div>
        <p className="text-slate-700 dark:text-slate-300">
          {t('aiCopilots.visuals.customerService.citationContent', 'Trích từ SOP-CS-04 (Quy chế Chăm sóc & Xử lý Khiếu nại, Mục 3.2 - Điều khoản Giao trễ do bất khả kháng): Trường hợp hàng trễ từ 24h-48h, nhân viên hỗ trợ được phép áp dụng chính sách: Miễn phí cước vận chuyển và tặng voucher ưu đãi 10% cho đơn hàng kế tiếp.')}
        </p>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs">
          <span className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
            ...
            {t('aiCopilots.visuals.customerService.suggestedResponseHeader', 'Câu trả lời AI gợi ý theo kịch bản [Đề xuất tham khảo để chuyên viên duyệt]:')}
          </span>
          <button
          ...
          >
            ...
            {isEditing
              ? t('aiCopilots.visuals.customerService.saveEditBtn', 'Lưu chỉnh sửa')
              : t('aiCopilots.visuals.customerService.editBtn', 'Chỉnh sửa câu chữ')}
          </button>
        </div>
      ...
      <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
          ...
          <span>
            {t('aiCopilots.visuals.customerService.checkpointNotice', 'Điểm chốt chặn: Nhân viên CSKH bắt buộc kiểm tra mã vận đơn trước khi duyệt gửi. AI không tự trả lời khách.')}
          </span>
        </div>
        ...
        <div>
          {responseStatus === 'draft' ? (
            ...
            >
              ...
              {t('aiCopilots.visuals.customerService.submitBtn', 'Chuyên viên duyệt & Gửi phản hồi')}
            </button>
          ) : (
            <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 px-3 py-2 rounded-xl border border-emerald-200 dark:border-emerald-800">
              ...
              {t('aiCopilots.visuals.customerService.submittedNotice', 'Đã gửi phản hồi cho khách hàng lúc 14:24 (SLA 4 phút)')}
            </div>
          )}
        </div>
      </div>
