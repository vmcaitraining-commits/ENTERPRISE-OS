...
export const FinalCtaSection: React.FC = () => {
  ...
  const deliverables = tRaw<string[]>('home.finalCta.deliverables', []);
        ...
        <div className="relative max-w-3xl mx-auto text-center space-y-8">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-[#06B6D4] text-xs font-bold uppercase tracking-wider border border-white/20">
              ...
              {t('home.finalCta.badge', 'SẴN SÀNG CHUYỂN ĐỔI')}
            </div>
            <h2 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">
              {t('home.finalCta.heading', 'Bắt đầu xây dựng hệ điều hành vận hành cho doanh nghiệp của bạn')}
            </h2>
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed max-w-2xl mx-auto">
              {t('home.finalCta.description', 'Không cần đầu tư chắp vá nhiều phần mềm rời rạc. Hãy bắt đầu với một kiến trúc thống nhất, chuẩn hóa quy trình và đưa AI vào vận hành an toàn.')}
            </p>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-white/5 border border-white/10 text-left max-w-xl mx-auto space-y-2.5">
            <div className="text-xs font-bold uppercase text-slate-300 tracking-wider">
              {t('home.finalCta.deliverableHeading', 'Doanh nghiệp của bạn sẽ nhận được:')}
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            ...
            >
              ...
              <span>{t('home.finalCta.ctaBooking', 'Đặt lịch tư vấn kiến trúc 1:1')}</span>
            ...
            >
              <span>{t('home.finalCta.ctaAssessment', 'Đăng ký đánh giá mức độ sẵn sàng AI')}</span>
            </button>
          </div>

          <div className="flex items-center justify-center gap-2 text-slate-400 text-xs pt-2">
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
            <span>{t('home.finalCta.privacyCommitment', 'Cam kết bảo mật 100% thông tin và dữ liệu doanh nghiệp')}</span>
          </div>
        </div>
      </div>
    </section>
