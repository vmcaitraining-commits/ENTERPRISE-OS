export interface BeforeAfterTask {
  task: string;
  before: {
    process: string;
    timeSpent: string;
...
export interface GovernanceData {
  ...
  rbacRules: string[];
  auditLogMechanisms: string[];
}

export interface TaskAutomationDivision {
  automatedTasks: {
    task: string;
    boundary: string;
    systemOutput: string;
...
export interface PilotRoadmapStep {
  week: string;
  phase: string;
  activities: string[];
  deliverable: string;
...
export interface CopilotDetailedInfo {
  slug: string;
  name: string;
  roleTitle: string;
  shortTagline: string;
  ...
  // 1. Hero Sample Output
  heroSample: {
    type: string;
    title: string;
    timestamp: string;
    summary: string;
    dataPoints: { label: string; value: string; status?: 'normal' | 'warning' | 'alert' | 'success' }[];
    sampleSnippet: string;
    actionRequired: string;
  };

  // 2. Before / After Comparison
  beforeAfter: BeforeAfterTask[];

  // 3. Demo Data Specification
  demoSpec: {
    title: string;
    sourceDescription: string;
    dataSources: { name: string; type: string; syncMode: string }[];
    disclaimer: string;
  };
  ...
  // 5. Automated Tasks vs Approval Checkpoints
  taskDivision: TaskAutomationDivision;

  // 6. Pilot Metrics
  metrics: PilotMetric[];

  // 7. Small Pilot Implementation Plan
  pilotPlan: {
    recommendedScope: string;
    duration: string;
    pilotSteps: PilotRoadmapStep[];
    goNoGoCriteria: string[];
  };
}

export const copilotDetailedMap: Record<string, CopilotDetailedInfo> = {
  ceo: {
    slug: 'ceo',
    name: 'AI CEO Copilot',
  ...
  rag: {
    ...
    heroSample: {
      ...
      dataPoints: [
        ...
        { label: 'Độ tin cậy trích dẫn', value: '100% dựa trên tài liệu gốc', status: 'success' },
    ...
    beforeAfter: [
      ...
      },
      {
        ...
        before: {
          process: 'Nếu dùng AI công cộng (ChatGPT thông thường), AI có thể tự bịa ra điều khoản không có thật trong công ty.',
        ...
        after: {
          process: 'Công nghệ RAG chỉ cho phép AI trả lời dựa trên các đoạn văn bản trích xuất được từ kho dữ liệu đã kiểm duyệt; không có tài liệu là từ chối trả lời.',
      ...
      },
      {
        ...
        after: {
          ...
          humanRole: 'Bảo vệ tuyệt đối các bí mật kinh doanh và dữ liệu nhạy cảm của công ty.'
        }
      }
    ],
    demoSpec: {
      ...
      disclaimer: 'Hệ thống RAG tuân thủ nguyên tắc nghiêm ngặt: Chỉ trả lời khi tìm thấy bằng chứng trong tài liệu nguồn. Nếu không có dữ liệu hoặc người hỏi không đủ quyền hạn, hệ thống sẽ thông báo rõ ràng thay vì tự suy diễn.'
    },
    governance: {
      allowedData: [
    ...
    metrics: [
      ...
      { name: 'Tỷ lệ an toàn bảo mật phân quyền RBAC', formula: '(Số vụ rò rỉ dữ liệu mật / Tổng số truy vấn) × 100%', dataSource: 'Báo cáo an ninh thông tin', frequency: 'Liên tục', targetBenchmark: 'Tuyệt đối 0% rò rỉ tài liệu ngoài phân quyền' }
    ],
    pilotPlan: {
      ...
      goNoGoCriteria: [
        '100% câu trả lời đều có trích dẫn link đến tài liệu nguồn cụ thể.',
        '0% sự cố nhân viên cấp dưới đọc được tài liệu mật của cấp quản lý.',
